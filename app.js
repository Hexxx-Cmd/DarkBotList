...
client.channels.get('683457272868896815').send(`:no_entry:  | User  \`${req.user.username}#${req.user.discriminator} ID (${req.user.id})\` Report The Bot \`${db.fetch(`botlar.${req.params.botID}.isim`)}\` Reason  \n**REASON:** \`${ayar['mesaj-1']}\``)
}
if(ayar['mesaj-2']) {
db.push(`botlar.${req.params.botID}.raporlar`, JSON.parse(`{ "rapor":"${ayar['mesaj-2']}" }`))
client.channels.get('683457272868896815').send(`:no_entry:  | ** User ** \`${req.user.username}#${req.user.discriminator} ID (${req.user.id})\` ** Reported The Bot ** \`${db.fetch(`botlar.${req.params.botID}.isim`)}\` ** Reason ** \n**REASON:** \`${ayar['mesaj-2']}\``)
}
...
https://snowbotlist.tk/bot/${db.fetch(`botlar.${req.params.ID}.isim`)}`) 
